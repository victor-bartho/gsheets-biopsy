...
def print_and_log(level_name, message):
    print(message)
    match level_name:
        case 'INFO':
            log.info(message)
...
def add_file_to_review_list(review_file_name, message=''):
...
print_and_log('INFO', 'Iniciando execução do script:')
try:
...
for file_name in os.listdir(directory_path):
    ...
    if os.path.isfile(old_file_path) and old_file_path.split('.')[-1].lower() == 'pdf':
        ...
        if info_extractor.validate_biopsy():
            print_and_log('INFO',f'Lendo arquivo: {file_name}')
            ...
            try:
                ...
                print_and_log('INFO',f'Informações do arquivo {file_name} (pedido: {biopsy_order_number}) '
                                     f'incluídas na planilha com sucesso')
            except Exception as e:
                ...
                log.error(f'Erro ao inserir informações do arquivo {file_name} na planilha de Google Sheets. Detalhes do '
                          f'erro:\n {e} \n\n Traceback para consulta:\n{traceback_register}\n\n')
                add_file_to_review_list(file_name, 'Realizar inserção manual de dados na planilha')
                continue
            ...
            try:
                ...
                log.error(
                    ...
                    f'erro:\n {e} \n\n Traceback para consulta:\n{traceback_register}\n\n')
                add_file_to_review_list(file_name, 'Iserção ok. Renomear arquivo manualmente')
                continue
        else:
